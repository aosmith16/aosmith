...
render_toc("2017-12-31-combining-many-datasets-in-r.Rmd")
```
...
# Why would I want to have to do this in R?
...
## Add identifying information in file path to dataset
...
These days I've been using the `map` functions from package **purrr** for this, mostly because the `map_dfr()` variant conveniently binds everything together by rows.
...
labs = labs[!labs %in% c("setup", "toc", "getlabels", "allcode", "makescript", "plot1")]
```
...
Here's the code without all the discussion.  Copy and paste the code below or you can download an R script of uncommented code [from here](/script/2017-12-31-combining-many-datasets-in-r.R).
