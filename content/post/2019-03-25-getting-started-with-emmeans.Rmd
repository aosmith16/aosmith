...
keywords:
  - tech
draft: TRUE
description: "Using emmeans for post hoc comparisons is very useful.  This post goes through some of the basics for those just getting started with the package."
---
...
I've put together some basic examples for using **emmeans**, meant to be a complement to the vignettes.  Specifically this post will go over some of the built-in options for some standard post hoc comparisons; I will write a separate post about building custom comparisons in **emmeans**.
...
I've made a small dataset to use as an example.  The response variable is `resp`, which comes from the log-normal distribution, and the two crossed factors of interest are `f1` and `f2`.  Each factor has two levels: a control called `c` as well as a second non-control level.
...
The model of interest is a linear model with a log-transformed response and the two factors and their interaction as explanatory variables.  This is the "true" model since I created these data so I'm skipping all model checks here (which I would not do in a real analysis).
...
The **emmeans** package has helper functions for common sets of comparisons (aka *contrasts*).  For example, we can do pairwise comparisons via `pairwise` or `revpairwise`, treatment vs control comparisons via `trt.vs.ctrl` or `trt.vs.ctrlk`, and even consecutive comparisons via `consec`.  

The available functions for doing standard comparisons are all listed in the documentation for `?"contrast-methods"`.

# [All pairwise comparisons](#all-pairwise-comparisons)

One way to use `emmeans()`, which I use a lot, is to use formula coding.  The type of comparison we want is on the left-hand side of the formula and the factor(s) that represent the group means we want to compare is on the right.  

This formula is defined in the `specs` argument.  We pass our model to the first argument in `emmeans()`, `object`.

Below I will do all pairwise comparisons for all group combinations.  The built-in function `pairwise` is put on the left-hand side of the formula and the groups we want comparisons for is on the right-hand side.  Since I'm doing all pairwise comparisons, this is the combination of `f1` and `f2`.
...
These results are also on the model scale (and we get the same message in this section), but [we'll want to put them on the original scale](#back-transforming-results).  
...
In the `contrast` column in the `contrasts` section we can see the expression of the comparisons has changed from additive comparisons (via subtraction) to multiplicative comparisons (via division).
...
We will almost invariably want to report confidence intervals for any comparisons.  We need a section function to get these.  Here is an example using the `confint()` function using the default 95% CI (the confidence level can be changed, see `?confint.emmGrid`).  I use the pipe to pass the `contrasts` into the `confint()` function.
...
One of the really nice things about `emmeans()` is that it is easy to get the results into a nice format for making tables or graphics of results.  This is because the results can be converted into a data.frame via `as.data.frame()`.

This is true for both `confint()` and `summary()`, although I'll just show `confint()` here.  After getting the confidence intervals I pipe the results into `as.data.frame()`.  I assign this object a name so I can use it for, e.g., making a graph.
...
While we *can* do all pairwise comparisons, there are certainly lots of times that the research question dictates that we only want a specific set of comparisons.  A common example of this is when we want to compare the levels of one factor within the levels of another.  Here I'll show comparisons among levels of `f1` for each level of `f2`.

The only thing that changes is the right-hand side of the `specs` formula.  The code `f1|f2` essentially translates to "compare levels of `f1` within each level of `f2`".
...
We use `rbind()` on the `contrasts` to adjust for the larger family of comparisons.  The default adjustment is Bonferroni, which can be much too conservative when the number of comparisons is large.  You can control the multiple comparisons procedure via `adjust`.

The results of `rbind()` can also be used in `summary()`,  `confint()`, and/or `as.data.frame()`.
...
The **emmeans** package also has built-in helper functions for comparing each group mean to the control mean.  If the control group is the in the first row of the `emmeans` section of the output, this set of comparisons can be requested via `trt.vs.ctrl`.
...
The `emmeans()` package also offers the option to do comparisons in two steps instead of in one step the way I did above by using a comparison function in the left-hand side of the `specs` formula.  I find this alternative most useful when doing custom contrasts, but I think it's good to introduce it now so it looks familiar.  This option keeps the estimated marginal means and the comparisons of interest in separate objects, which may be attractive in some situations.
...
We can follow the `contrast()` argument with `summary()` or `confint()` and then put the results into a data.frame for plotting/saving.  Again, I think the real strength of `contrast()` comes when we want custom contrasts, and I'll demonstrate these in my next post.
