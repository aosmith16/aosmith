...
I drive ~70 commuting miles each day, and we wanted a car we could afford that was competitive with the CRX on gas mileage.  The CRX got solidly just over 40 mpg.

```{r, echo = FALSE, fig.cap = "The dog wonders how the gas mileage will compare"}
...
The workbook has a sheet for every year driven (into the future! `r emo::ji("laugh")`) plus service records.
...
We calculate gas mileage (`mpgcalc`) based on recorded gallons and mileage driven.  We also record what the car estimated the gas mileage would be (`mpggage`).  

```{r}
head(fiesta2017)
```

Here's my first chance to use functions from package **skimr** to get a quick look at the variables in the dataset. (The locale change is to try to get [histograms to display correctly](https://github.com/ropenscilabs/skimr/issues/136), which works in the RMD but not in HTML.)
...
I'll need to convert `date` to a date and remove a missing value before proceeding (looks like we forget to enter the mileage on one gas stop).
...
I'll have to remove the odd data point as I'm not sure what the mistake is (changing the mileage to `3` instead of `6` seems a reasonable guess but that still led to something unreasonable large).
...
Here's a plot of calculated gas mileage over the year, plotted via **ggplot2**.  I put a horizontal line at 40 mpg and one at the annual average observed mpg.
...
It's fun to watch how the estimated mpg is affected by conditions while I'm driving (e.g., AC, tail-wind, defrost), but it looks like whatever that algorithm is tends to overestimate the gas mileage.  But not always!
