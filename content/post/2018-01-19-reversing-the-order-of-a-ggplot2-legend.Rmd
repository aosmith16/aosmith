...
It's always nice to get good questions in a workshop. It can help everybody get a bit of extra learnin' in.

I do a **ggplot2** workshop for graduate students in my College every spring.  The first half is focused on the terminology and understanding the basics of how to put a plot together (I remember as a beginner feeling like I was throwing darts at things to see what stuck when deciding if something should go inside or outside aes() `r emo::ji("dart")` ). 
...
I have a "results" dataset I use in one of the "final" graphs, displaying the results from some statistical model used to answer a question about differences in mean response between some groups vs a control group.
...
In the workshop, making the graph of these results is done in many stages to demonstrate how scales and themes and flipping coordinates works.  At one point during the workshop the graph looks like this.
...
One year I had a student ask a great question about the order of the legend.  The order of the horizontal lines in the plot is exactly opposite of the order of the lines in the legend.  He pointed out that the plot would feel "tighter" if the orders matched; I agreed.

Now, a lot of time the answer to "how do I change the order of a categorical variable in **ggplot2**" is "change the data to change the plot."  (I've use `forcats::fct_inorder` a *lot* for getting the levels of variables like month names in the right order in plots.)
...
...both the order of the lines in the plot and the legend flip orders.
...
Back to the drawing board (and some time online), it turned out the answer was to reverse the legend within **ggplot2**.  There is a "reverse" argument in the guide_legend() function.  This function can be used either inside a scale_*() function with the "guides" argument or within guides().
