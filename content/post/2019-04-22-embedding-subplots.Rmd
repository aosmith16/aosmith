...
I want the x axis of each plot to encompass the entire range of `Petal.Width`, so my first step was to pull out that information.  These values will be the x axis limits (with some extra added to make sure all the histogram bars fit).
...
I'll also need to set the `binwidth` to make the histogram bars are the same width for each plot.  I arbitrarily chose `0.25` but seeing the final produce I could have used narrower bars.  This value is also what I used to pad the x axis limits to make sure all the bars show in every plot.
