...
I gave an introduction to generalized linear models this quarter.  You can't really cover that much in two weeks, but I did manage to talk to students about zero inflation with count distributions for a few minutes.  Outside of talking about approaches when you have zero inflation, the main thing I wanted to stress is that having a lot of zero values does not necessarily mean you have zero inflation.
...
Here is a histogram of these data.  I've added in info on the proportion of the 200 values that are 0 as well as the maximum count in the dataset.
...
The example above demonstrates a model fitting the data.  Let me finish by fitting a model to data that has more zeros than expected by the model.  This is as simple as fitting a Poisson GLM instead of a negative binomial GLM to the data above.
