...
Package **HMMpa** is for drawing random samples from the generalized Poisson distribution.
...
R uses the parameterization of the negative binomial where the variance of the distribution is $\lambda + (\lambda^2/\theta)$.  In this parameterization, as $\theta$ gets small the variance gets big.  Using such a small value for theta will generally mean the distribution of counts can have many zeros as well as a few large counts

I make random draws from this distribution using `rnbinom()`.  The `mu` argument is the mean and the `size` argument is theta.  
...
Here is a histogram of these data.  I've annotated the plot with the proportion of the 200 values that are 0 as well as the maximum observed count in the dataset.  There are lots of zeros!  But this is not zero-inflated, because we can expect to have many 0 values under this distribution.
...
From my understanding, the generalized Poisson distribution can have heavier tails than the negative binomial.  It can have more extreme maximum counts as well as lots of zeros.
...
Here is a histogram of these data.  Just over 50% of the values are zeros but the maximum count is over 1000! `r emo::ji("boom")`
...
All the simulations above show us is that some distributions *can* have a lot of zeros.  In any given scenario, though, how do we check if we have *excess* zeros?  Excess zeros is having more zeros than expected by the distribution we are using for modeling.  If we have excess zeros than we may either need a different count distribution or we could think about models that specifically address zero-inflation.

The key to checking for excess zeros is to estimate the number of zeros you would expect to see if the fitted model were truly the model that created your data.

# Simulate negative binomial data

To demonstrate this, I'll make a dataset based on a negative binomial model with a single, continuous explanatory variable.  
...
Using `dpois()`, the number of zeros given be the Poisson model is 0. `r emo::ji("open_mouth")` These data are zero-inflated compared to the Poisson distribution, and I would need a different option for modeling these data.
