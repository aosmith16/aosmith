...
I use simulations to check my intuition and understanding of statistical theory.  When someone tells me with great certainty "I don't need to meet that assumption because [fill in the blank]", I tend turn to simulations rather than textbooks to check.

I like simulations for the same reasons I like building Bayesian models and using resampling methods (i.e., Monte Carlo) for inference.  Building the simulation increases my understanding of the problem and makes all the assumptions clearer to me because we use them explicitly.

# Simulate, simulate, dance to the music

I find simulations so useful for my own understanding, and would like to use them more in teaching and consulting as I think the could help folks understand their models better.  I haven't managed to fit it in so far, but it's always on my mind.

It was when I was learning to using mixed models in R (having learned them in SAS) when I started using simulations heavily, and mixed models is where my brain jumps to when I thought about a "simulation" post.  However, I thought it worthwhile to start out with a simulation from a simpler model to demonstrate how things work before increasing the complexity.  Today I'll go over a simulation from a two-group linear model and then work up to linear mixed models and generalized linear mixed models in subsequent posts.

# The statistical model

**Warning: Here there be equations.**  

If your brain says "I think this section must not pertain to me" after seeing the mathematical notation (like mine does), you can jump right down to the R code in the next section.

A simulation for a linear model is based on the statistical model. The statistical model is an equation that describes the processes we believe gave rise to the observed response variable.  It includes parameters to describe the assumed effect of explanatory variables on the response variable as well as a description of any distributions associated with processes we assume are random variation. (See Stroup's 2015 "Generalized Linear Mixed Models" book for a more in-depth description).  So the statistical model, which can look very math-y, is where we write down the exact assumptions we are making when we fit a linear model to a set of data.  Once we know those, we can simulate data based on those assumptions.
...
I'll set my seed for the random number generation so I can replicate this you simulation exactly at home.

```{r}
set.seed(16)
```

I'll start out by defining what the "truth" is in the simulation by setting the parameter values.  Here's what I'll do today.
